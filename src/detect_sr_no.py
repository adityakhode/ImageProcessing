...
from sr_no_service import build_default_detector, SRNoDetectionResult
...
def extract_numbers(img) -> Optional[int]:
    """Extract SR number from a voter block image.

    Args:
        img: Input image (BGR format).

    Returns:
        The detected SR number as an integer, or None if detection fails.
    """
    detector = _get_default_detector()
    result: SRNoDetectionResult = detector.detect(img)
    return result.sr_no
...
if __name__ == "__main__":
    ...
    # Extract SR number from the image
    sr_no = extract_numbers(test_img)

    # Print the extracted SR number
    print(f"Extracted SR Number: {sr_no}")
